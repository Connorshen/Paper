...
def load_mnist(flatten=True, one_hot=True, ratio=1.0, digits=np.arange(0, 10)):
    train_image, train_label, test_image, test_label = load_file(DataConfig.MNIST_TRAIN_IMAGE_PATH,
                                                                 DataConfig.MNIST_TRAIN_LABEL_PATH,
                                                                 DataConfig.MNIST_TEST_IMAGE_PATH,
                                                                 DataConfig.MNIST_TEST_LABEL_PATH,
                                                                 one_hot=one_hot)
...
def load_fashion(flatten=True, one_hot=True, ratio=1.0, digits=np.arange(0, 10)):
    train_image, train_label, test_image, test_label = load_file(DataConfig.FASHION_TRAIN_IMAGE_PATH,
                                                                 DataConfig.FASHION_TRAIN_LABEL_PATH,
                                                                 DataConfig.FASHION_TEST_IMAGE_PATH,
                                                                 DataConfig.FASHION_TEST_LABEL_PATH,
                                                                 one_hot=one_hot)
    if flatten is not True:
        train_image = train_image.reshape(-1, 1, 28, 28, )
        test_image = test_image.reshape(-1, 1, 28, 28, )
...
def fashion_loader(batch_size=32, shuffle=True, flatten=True, one_hot=False, digits=np.arange(0, 10)):
    train_image, train_label, test_image, test_label = load_fashion(flatten=flatten, one_hot=one_hot, digits=digits)
    train_dataset = TensorDataset(torch.from_numpy(train_image), torch.from_numpy(train_label).long())
    test_dataset = TensorDataset(torch.from_numpy(test_image), torch.from_numpy(test_label).long())
    train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=shuffle)
    test_loader = DataLoader(test_dataset, batch_size=batch_size, shuffle=False)
