...
file_name = "train_history/compre_convergence.mat";
compre_convergence = cell(trial,4);% [rl_check_points,rl_best_train_result,rl_batch_check_points,rl_batch_best_train_result]
% 初始化参数
digits = 0:2;
...
for i = 1:trial
    ...
    compre_convergence{i,1} = rl_check_points;
    compre_convergence{i,2} = rl_best_train_result;
    % 训练rl_batch
    net = init_net(init_para);
    disp("init net success")
    ...
    compre_convergence{i,3} = rl_batch_check_points;
    compre_convergence{i,4} = rl_batch_best_train_result;
end
save(file_name,"compre_convergence","-v7.3");
