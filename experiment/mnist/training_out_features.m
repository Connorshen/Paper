clear
clc
% 无关的参数
trial = 1;
data_ratio = 1;% 数据集比例
test_early_stopping = -1;% 测试的时候提早break的step，不想提早结束的话取-1
train_early_stopping = 1000;% 训练的时候提早break的step，不想提早结束的话取-1
file_name = "train_history/compare_out_features.mat";
compare_out_features = cell(1,2);% [rl_check_points,rl_batch_check_points]
% 初始化参数
digits = 0:9;
in_features_cpl = 2560;
out_features_cpls = [5000,10000,30000,50000,100000];
verify_step = 100;
get_lr_step = 100;
get_lr_batch = 100;
n_neuron_cluster = 10;
% 加载数据集
data = load_mnist_data(digits,data_ratio);
disp("load data success");
for i=1:length(out_features_cpls)
    out_features_cpl = out_features_cpls(i);
    for j=1:trial
        init_para = init_paramter(digits,in_features_cpl,out_features_cpl,n_neuron_cluster,verify_step,get_lr_step,get_lr_batch);
        disp("init para success")
        init_para
        rand('state',j);
        net = init_net(init_para);
        disp("init net success")
        [rl_check_points,rl_best_train_result] = rl_trainer(init_para,net,data,train_early_stopping,test_early_stopping);
        compare_out_features{i,j*2-1} = rl_check_points;
        compare_out_features{i,j*2} = rl_best_train_result.init_para.out_features_cpl;
    end
end
save(file_name,"compare_out_features","-v7.3");
plot_out_features()